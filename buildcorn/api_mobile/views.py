        ...
        if input_data["type"] == 'Quality':
            ...
            try:
                for que in input_data.get('question'):
                    ...
                    query, created = self.queryset.get_or_create(question=question, quality_checklist=quality_checklist_name)
                    if created:
            ...
            try:
                for que in input_data.get('question'):
                    question = Question.objects.get(id=que["id"])
                    query, created = self.queryset.get_or_create(question=question, safety_checklist=safety_checklist_name)
                    if created:
                        query.status = que.get('status',None)
                        query.reason =  que.get('reason',None),
                        query.save()
